...
export function ImageUploadModal({ isOpen, onClose, onUploadComplete }: ImageUploadModalProps) {
  ...
  const [exitndroImage, setExitndroImage] = useState<File | null>(null);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (exitndroImage) {
        setUploadProgress('Uploading Exitndro deployment image...');
        await uploadImage(exitndroImage, 'exitndro-deployment.jpg');
      }
      ...
      setUploadProgress(`Upload failed: ${error.message || 'Please try again.'}`);
      setTimeout(() => {
        setUploadProgress('');
      }, 5000);
    } finally {
  ...
  const hasAnyFile = heroImage || focus1Image || focus2Image || focus3Image || exitndroImage;
        ...
        <div className="p-6 space-y-6">
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
            <p className="text-sm text-amber-900">
              Upload images for your homepage. All images are optional. Recommended size: Hero background (1920x1080), Focus images (800x800).
            </p>
          </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  type="file"
                  accept="image/*"
                  onChange={(e) => handleFileChange(e, setExitndroImage)}
                  className="hidden"
                  id="exitndro-upload"
                  disabled={uploading}
                />
                <label
                  htmlFor="exitndro-upload"
                  className="flex items-center justify-center gap-3 p-6 border-2 border-dashed border-slate-300 rounded-lg hover:border-amber-500 hover:bg-amber-50 transition-colors cursor-pointer"
                >
                  {exitndroImage ? (
                    <>
                      <Check className="w-5 h-5 text-green-600" />
                      <span className="text-slate-900 font-medium">{exitndroImage.name}</span>
                    </>
                  ) : (
                    <>
                      <Upload className="w-5 h-5 text-slate-400" />
                      <span className="text-slate-600">Click to upload Exitndro deployment image</span>
                    </>
                  )}
                </label>
              </div>
